...
Specify the symbol distance to the edge with `label_distance`, the angle at which the edge labels are displayed with `label_angle`, and the maximum number of ros in each node table with `n`. The latter is particularly important when one of the tables has many attributes.
